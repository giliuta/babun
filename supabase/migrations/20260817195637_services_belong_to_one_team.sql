...
-- лежащих здесь файла — `20260821120000_drop_service_unit.sql` и
-- `20260821140000_service_rows.sql` — обе проекции ролей
-- (`list_dispatcher_services_safe()`, `list_master_services_safe()`) строят с
-- `s.team_id`. Ссылка по именам функций, а не по номерам строк: номера уезжают
-- от любой правки соседнего файла и через неделю указывают в пустоту.
...
--
-- ЧТО С ОСТАЛЬНЫМИ ТРЕМЯ ПРОД-МИГРАЦИЯМИ ПО УСЛУГАМ — проверено и записано
-- здесь, чтобы следующий заход не «восстановил» и не снёс их задним числом:
...
--     ФАЙЛОВ НЕ ТРЕБУЮТ: оба перекрыты, `create or replace` тех же функций стоит
--     в файлах от 21.08, и чистая среда получает СРАЗУ конечные тела. Колонка
--     `color` в репозитории есть с 20260624_001 (`not null default '#3b82f6'`),
--     заводить нечего.
--   · 20260819095608 `service_unit_of_measure` — ФАЙЛ ЗАВЕДЁН, лежит рядом
--     (`20260819095608_service_unit_of_measure.sql`). Первое суждение («колонку
--     всё равно снесли 21.08, значит файл не нужен») было ошибкой: между 19.08 и
--     21.08 стоит 20260820120000_drop_service_invoice_note.sql, а он пересоздаёт
--     `list_dispatcher_services_safe()` как `language sql` с `s.unit` в
--     проекции. Такие тела Postgres проверяет ПРЯМО ПРИ CREATE, поэтому на
--     чистой базе цепочка обрывалась с «column s.unit does not exist».
--     Колонка живёт три файла и уходит в 20260821120000 — итоговая схема от неё
--     не меняется, но без неё промежуточные миграции не применяются.
