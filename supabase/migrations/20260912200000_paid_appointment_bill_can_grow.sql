-- ОПЛАЧЕННУЮ ЗАЯВКУ МОЖНО ДОПИСАТЬ РАБОТАМИ (владелец 2026-09-12: «не могу
-- почему-то сейчас сохранить счёт, допустим, когда оплачено — реши всё»).
--
-- Что было. Бригада приехала, получила €160 наличными, доделала ещё работ —
-- мастер дописывает услуги в записи, «Итого» становится €280 и жмёт
-- «Сохранить». Сторож `protect_paid_appointment_finance` отвечал «Сначала
-- отмените оплату или оформите возврат по заявке»: у заявки с проводкой
-- оплаты замораживались ВСЕ поля работы и цены — итог, услуги, скидки. То
-- есть единственный способ выставить остаток — снять уже полученные деньги,
-- переписать запись и принять их заново. На объекте этого никто не делает.
--
-- Почему заморозка вообще была. `reconcile_appointment_finance` выводит сумму
-- проводки из СТАТУСА: у `paid` цель = итог − предоплата, у `partial` = уже
-- полученное. Поднять итог, оставив статус `paid`, значило бы дописать в
-- финансы доход €120, которых никто не получал. Заморозка закрывала дыру
-- целиком — вместе с честным сценарием.
--
-- Что теперь. Статус идёт за итогом: полученное остаётся полученным, а
-- заявка со €160 из €280 становится `partial` — цель проводки у `partial`
-- равна уже полученному, поэтому в финансах НЕ МЕНЯЕТСЯ НИ ОДНОЙ СТРОКИ.
-- Разница просто становится долгом, как у любой частично оплаченной заявки.
-- Опустить итог НИЖЕ полученного по-прежнему нельзя («Полученная сумма больше
-- итога заявки») — это возврат, а не правка.
--
-- Замороженным осталось всё, что действительно ломает деньги: компания,
-- клиент, команда, исполнитель, день, способ оплаты, предоплата и сам леджер
-- платежей.
...
declare
  ...
  -- Полученное по НОВОЙ строке, без оглядки на статус: им меряется и правка
  -- счёта, и то, каким статус обязан стать.
  received_now numeric := 0;
  -- Статус сам сходил за изменившимся итогом — эта смена разрешена.
  status_follows_total boolean := false;
  -- Работы и цену править можно: деньги уже получены, леджер не тронут, и
  -- новый итог их покрывает.
...
begin
  ...
  -- СТАТУС ИДЁТ ЗА ИТОГОМ. Итог вырос выше полученного — заявка становится
  -- частично оплаченной; опустился до полученного — оплаченной. Деньги при
  -- этом не трогаются: цель проводки у `partial` равна уже полученному, и
  -- `reconcile_appointment_finance` не пишет ни строки.
  --
  ...
  -- Правка работ и цены по оплаченной заявке разрешена, пока леджер платежей
  -- не тронут и новый итог покрывает полученное.
  bill_edit_allowed :=
  ...
  if new.tenant_id is distinct from old.tenant_id
     ...
     -- (`bill_edit_allowed`) — см. шапку миграции.
