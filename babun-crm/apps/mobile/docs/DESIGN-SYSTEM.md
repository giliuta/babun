...
- **Detent sheet** (`DetentSheet` — the nonmodal sibling for LIVE-APPLY surfaces): renders IN the screen tree (not Modal); medium detent ~55% has NO scrim and the content behind stays alive and tappable — every tap inside the sheet visibly rebuilds the list behind it; dragging to large (~92%, capped below the status bar) fades a scrim over the top stretch; scrim-tap returns to medium; the CTA footer is pinned to the SCREEN bottom in its own layer so the live counter is visible on any detent; the body scrolls on every detent, only the grabber/header resizes. Modal editing keeps `BottomSheet`; live filtering uses `DetentSheet`.
...
- **«Лента времени»** (`PeriodBand`) — canonical period control: one 48pt track of the last 12 months, each column engraved with record density (ink 20% bars over a hairline baseline); tap = whole month, horizontal stroke = month range (per-column snap + selection haptics), tap on the single selected month = «Всё время»; selected months print as an ink band with white engraving (two-plane clip). Exact days and ranges beyond the window live in the С–До wheels behind the section's value button. Grid preset pairs are retired.
...
- **Clients:** Display «Клиенты» + gear; list rows = avatar + name (Headline) + meta (Body) + money/status badge; filter summary bar with removable accent chips → nonmodal `DetentSheet` «Фильтры» (the «Оттиск + Лента» canon: all sections visible, ink selection, `PeriodBand`, live list behind).
