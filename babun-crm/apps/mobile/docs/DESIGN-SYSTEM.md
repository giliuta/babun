...
- **Clients:** Display «Клиенты» + gear; list rows = avatar + name (Headline) + meta (Body) + money/status badge; filter summary bar with removable accent chips → full-page `BottomSheet` «Фильтры»: five single-dialect rows (Статус · Период · Метка · Теги · Команда), all picking in popups (оттиск multi-pick rows + contextual counts; period = Finances split), live CTA counter + active-summary caption.
