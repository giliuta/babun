...
- **Шва для телеметрии ошибок записи СЕЙЧАС НЕТ.** `sync-error-bus.ts` был
  агрегатором, в который телеметрия инжектилась через `setSyncErrorTelemetry`,
  но инжектить было некому: реализация оставалась no-op с самого начала, и
  2026-08-25 модуль снесён вместе с остальным мёртвым кодом. Ошибки записи
  человек видит только в красной плашке «Ошибка синхронизации».
  Когда телеметрию будут включать по-настоящему — шов заводится заново,
  уже вместе с рабочим приёмником.
