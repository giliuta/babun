...
13. **БЕРИ ГОТОВОЕ, НЕ ПИШИ ВТОРОЕ.** Новый экран, блок, лист или ряд заводится
    только после того, как поиском доказано, что готового нет. Одна сущность —
    одно тело; вторая дверь — реэкспорт в 5 строк. Полный порядок и реестр
    канона — раздел **Canon Reuse** ниже; он обязателен к чтению перед любой
    правкой UI

12.1 **НА СИМУЛЯТОРЕ ДВА ПРИЛОЖЕНИЯ — СМОТРЕТЬ НАДО В `com.babun.crm.dev` (2026-08-27).**
    `com.babun.crm` («Babun») несёт ВШИТЫЙ бандл и от правок кода не меняется
    никогда. `com.babun.crm.dev` («Babun Dev») ходит в Metro — только он
    показывает то, что ты сейчас написал. 27 августа на этом потеряно двадцать
    минут: правки уходили в коммиты, а на экране стояла старая версия, и я
    трижды объявил владельцу «не применилось», хотя применилось всё.

    Порядок при любом «на экране не то, что в коде»:
    1. `curl -s -o /dev/null -w "%{http_code}" http://localhost:8081/status` —
       Metro жив? Он падает молча, приложение при этом продолжает крутить
       последний загруженный бандл и выглядит рабочим.
    2. Поднять с ЧИСТЫМ кэшем: `bun run start --clear` из `apps/mobile`.
       Без `--clear` Metro отдаёт старую сборку («Bundled 64ms (1 module)» —
       признак кэша, честная пересборка это ~4000 модулей).
    3. `xcrun simctl terminate <udid> com.babun.crm.dev`, затем
       `xcrun simctl openurl <udid> "babun://expo-development-client/?url=http%3A%2F%2Flocalhost%3A8081"`.
    4. Перезапуск из фона НЕ перечитывает бандл — нужен именно terminate.

    Проверить догадку до перезапусков можно так: скачать бандл
    (`curl "http://localhost:8081/node_modules/expo-router/entry.bundle?platform=ios&dev=true"`)
    и поискать в нём свою новую строку. Есть — виновато приложение, нет —
    Metro.

## Canon Reuse — БЕРИ ГОТОВОЕ, НЕ ПИШИ ВТОРОЕ (LOCKED 2026-09-10)

Владелец 2026-09-10, после сплошной проверки архитектуры: «если я прошу
добавить блок с клиентом — добавляй такой же блок, какой мы уже разработали,
по дизайну по всему; не нужно думать что-то новое… если я показываю в одном
месте, значит то же самое будет показывать в другом месте, мне не надо разные
дизайны».

Это правило старше вкуса: продукт уже дважды расходился сам с собой —
«добавить объект» и «править объект» стали двумя разными формами, а кнопка
одного и того же действия получила три радиуса. Каждое такое расхождение
рождалось одинаково: агент писал НОВОЕ вместо того, чтобы найти готовое.

**Порядок обязателен, шаги не пропускаются.**

1. **СНАЧАЛА ИЩИ, ПОТОМ ПИШИ.** До первой строки нового экрана, блока, листа
   или строки-ряда — найди существующее:
   `grep -rn "<то-же-по-смыслу>" apps/mobile/src apps/mobile/app`. Нашёл —
   импортируй. Не нашёл — ищи второй раз другими словами (RU-подпись, имя
   сущности, имя примитива), и только потом заводи новое.
2. **ОДНА СУЩНОСТЬ — ОДНО ТЕЛО.** Страница, форма или лист сущности пишется
   ОДИН раз. Создание и правка — один компонент с режимом (`id === "new"`,
   `mode: "create" | "edit"`), а не два похожих файла: два файла расходятся
   в первый же редизайн, и это уже случалось.
...
4. **БЛОК ВЫГЛЯДИТ ОДИНАКОВО ВЕЗДЕ.** Блок «Клиент», «Объект», «Заметка»,
   «Услуги», «Метка», «Время», «Файлы», «Оплата» — один компонент на весь
   продукт: тот же примитив, та же подпись, та же иконка, тот же отступ
   подписи, тот же порядок строк. Захотелось «здесь чуть иначе» — это правка
   общего блока (и тогда она видна везде) либо проп, а не второй блок.
5. **ЛИСТ — ТОЛЬКО `BottomSheet`.** Сырой `<Modal>` из react-native в новом
   коде запрещён: он не знает канона шапки, скрима, высоты, клавиатуры и
   нижнего отступа. Выбор из справочника — `PickerSheet` / `ValuePickerSheet`;
   выбор из сотен строк — лист с поиском (образец `ClientPickerSheet`);
   подтверждение — `confirmThen` / `chooseOption`.
5.1 **ВРЕМЯ И ДАТА — ОДИН БАРАБАН В ДВУХ ОПРАВАХ (LOCKED 2026-09-10).**
   Контрол один на продукт: `TimeWheelPair` (момент и длительность),
   `TimeRangePicker` (пара границ), нативный spinner (дата). Оправа зависит
   от того, откуда его открывают:
   • **со страницы** — канонический лист (`DateWheelSheet`, `HourRangeSheet`);
   • **из листа** — тот же барабан РАСКРЫВАШКОЙ под строкой
     (`ValueRow expanded` + барабан ниже, образец `TeamScheduleSheet`), потому
     что двух модальных листов в одном кадре iOS не показывает.
   Компактный нативный пикер в строке (`display="compact"`) ЗАПРЕЩЁН: тап по
   нему ничего не открывает — он дорисовывает второй крошечный контрол у
...
6. **РЯД — ТОЛЬКО `card-rows` И `SectionCard`.** `FieldRow`, `ChoiceRow`,
   ...
   одном сценарии смешивать нельзя.
7. **КНОПКА ОДНА.** `GradientButton` (главная) и `Button variant="secondary"`
   (второй вид) — оба `t.radius.card`. Своя `Pressable` в роли кнопки — только
   если это ряд списка, и тогда это `card-rows`. Голый «+» и floating FAB
   запрещены (стандарт «Добавить»). Одно действие — одно слово на весь
   продукт: применить набранное в листе — «Применить»; создать сущность —
   «Создать N»; добавить в справочник — «Добавить N».
8. **МЁРТВОЕ СНОСИТСЯ В ТОМ ЖЕ ЗАХОДЕ.** Компонент без импортов врёт: он
   выглядит каноном и им не является (так `RefListScreen` и
   `InlineServiceCreate` пролежали мёртвыми, пока их комментарии обещали
   «один экран на все справочники»). Нашёл — либо подключи, либо удали.

**Что проверять перед «готово» (грепы обязательны, не на глаз):**

```bash
# второй сырой лист вместо BottomSheet
grep -rln "<Modal" apps/mobile/src apps/mobile/app | grep -v components/ui/BottomSheet
# радиус числом вместо токена
grep -rnE "borderRadius: *[0-9]|rounded-(xl|2xl|3xl|md|lg)" apps/mobile/src apps/mobile/app
# компонент без импортов
grep -rn "ИмяНовогоКомпонента" apps/mobile/src apps/mobile/app
# файл вырос за 400 строк
find apps/mobile/src apps/mobile/app -name "*.tsx" -exec wc -l {} + | awk '$1>400'
```

**Реестр канона — где лежит единственное тело.** Строка добавляется сюда в
том же коммите, в котором появляется новая сущность.
