...
// Нативная косметика (display/locale/themeVariant/accentColor) принимается и не
// читается — в браузере вид выбора задаёт сам браузер.
//
...
export interface DateTimeInputProps {
   ...
   *  дату и время он спрашивает ДВУМЯ строками (см. страницу записи
   *  `/book` и листы финансов — `mode="date"` рядом с `mode="time"`), а обратный
  ...
  style?: unknown;
  /** Нативная косметика, которую места вызова РЕАЛЬНО передают, — принимаем и
   *  не читаем: вид выбора в браузере задаёт сам браузер. Прополото 2026-08-25
   *  (`is24Hour`, `textColor` не передавал никто): проп, которого не ставит ни
   *  одно место вызова, ничего не удерживает — он просто обещает совместимость,
   *  которую некому проверить. */
  display?: string;
  locale?: string;
  themeVariant?: string;
  accentColor?: string;
