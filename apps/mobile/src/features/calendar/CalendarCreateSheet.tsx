...
export function CalendarCreateSheet({
  ...
  // читается как поломка продукта. Офлайн первым — остальные придирки
  // бессмысленны, пока записать всё равно некуда.
  const reason = !online
    ? "Нет связи — календарь не создать"
    : !name.trim()
      ? "Дайте календарю название"
      : null;
  const canSave = reason == null && !create.isPending;
  const footerNote = failure ?? (create.isPending ? null : reason);
