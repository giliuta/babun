...
export function TimezoneSheet({
  ...
  // Прочие города той же группы: человек ищет СВОЙ город, а подписана группа
  // одним. Без этой строки житель Тбилиси не понял бы, что «Dubai (UTC+4)» —
  // это он и есть.
  // Убираем из перечня тот город, которым группа УЖЕ подписана в барабане, —
  // иначе он стоит дважды в двух строках подряд.
  const shown =
    ownCity && group.cities.includes(ownCity) ? ownCity : tzLabel(group.zone);
  const others = group.cities.filter((c) => c !== shown);
  ...
  return (
    <BottomSheet
      ...
      onClose={onClose}
      title="Часовой пояс"
      // Полэкрана: барабан + часы + кнопка. Список на 86% высоты был здесь
      // до 27 августа — от него владелец и отказался.
      maxHeightRatio={0.5}
      footer={
        <View className="px-5">
          <Button
    ...
    >
      {inherited ? (
        ...
        >
          Сейчас этот календарь берёт пояс компании. Остальные не изменятся.
        </Text>
      ) : null}
      ...
      {searching ? (
        ...
        <View style={{ alignItems: "center", marginTop: 12 }}>
          {/* ПОДЛОЖКА ПОД БАРАБАНОМ. Без неё кольцо висит на белом и читается
              просто как три строки текста, а не как крутящийся контрол. */}
          <View
            style={{
              // ВЫСОТА ЗАДАНА ЗДЕСЬ, А НЕ ОТДАНА БАРАБАНУ. Своей высоты
              // колонка не удержала: лента растянулась на семь рядов, распёрла
              // лист и вытолкнула «Применить» за нижнюю кромку экрана — кнопки
              // на листе просто не стало. Обрезаем сами.
              height: WHEEL_H,
              borderRadius: t.radius.card,
              backgroundColor: t.canvas,
              overflow: "hidden",
          ...
          {/* ЖИВЫЕ ЧАСЫ СО СМЕЩЕНИЕМ В СКОБКАХ (владелец 2026-08-27). Сверить
              их с часами в статус-баре — самый быстрый способ убедиться, что
              продукт считает день так же, как ты. Моноширинно: иначе цифры
              пляшут при кручении барабана. */}
          <Text
            maxFontSizeMultiplier={1.2}
            style={{
              marginTop: 12,
          ...
          >
            {zoneClock(group.zone)}{" "}
            <Text style={{ fontSize: 17, fontWeight: "500", color: t.sub }}>
              ({utcLabel(group.zone)})
            </Text>
          </Text>
          {others.length ? (
