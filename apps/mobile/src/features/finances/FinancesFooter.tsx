...
export function FinancesFooter({
  ...
  const button =
    view === "accounts" ? (
      ...
      // подделкой. Рождается он ТОЛЬКО у дохода С КЛИЕНТОМ (триггер выходит при
      // client_id null), а форма операции клиента не знает — доход записывался,
      // чек не появлялся никогда. Клиент уже есть у ЗАПИСИ, поэтому кнопка
      // ведёт в «Долги» — список работ, ждущих денег: оплату принимают в самой
      // записи (канон владельца), и чек рождается сам. Выслать уже выданный чек
      // можно из него самого.
      <GradientButton label="Принять оплату" onPress={onAcceptPayment} />
    ) : view === "documents" ? (
