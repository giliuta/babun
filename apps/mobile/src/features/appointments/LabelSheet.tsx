...
export function LabelSheet({
  ...
  value: string | null;
  /** `null` — вернуться к метке дня (тап по уже выбранной строке). */
          ...
          options.map((option) => {
            ...
            return (
              <Pressable
                ...
                onPress={() => {
                  haptics.tap();
                  // Тап по уже выбранной снимает СВОЮ метку и возвращает
                  // запись к метке дня — тот же жест, что снимает метку дня
                  // в календаре.
                  onPick(chosen ? null : option.name);
                }}
