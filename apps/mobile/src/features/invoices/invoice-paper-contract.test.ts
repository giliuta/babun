...
describe("бумага инвойса и PDF печатают одну модель", () => {
  ...
  test("продавец печатается по одному разу в каждом рендере", () => {
    const count = (code: string) =>
      [...code.matchAll(/\bdoc\.seller\.name\b/g)].length;
    assert.equal(count(pdfCode), 1, "в PDF продавец напечатан дважды");
    assert.equal(count(paperCode), 1, "на экране продавец напечатан дважды");
  });

  // ЗАПАСНЫЕ СЛОВА ПУСТОТЫ — В ОБОИХ РЕНДЕРАХ. Экран говорил «Без названия» и
  // «Позиции пока не заполнены», а PDF в тех же местах молчал.
