...
// «Метка», а не «Город»: колонка `clients.city` хранит МЕТКУ — её
// проставляет запись в день с меткой, и написать туда можно что угодно
// («Греция», «День ног»), а не только город.
const HEADER = ["Имя", "Телефон", "Метка", "Долг", "Теги"] as const;
