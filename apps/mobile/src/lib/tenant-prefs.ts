...
/** Ключи, которые принадлежат ЧЕЛОВЕКУ, а не компании, и потому тоже переживают
 *  переход — хотя компанию в имени не называют.
 *
 *  Пока здесь один, и он попал сюда не по замыслу, а по разбору: реестр
 *  напоминаний. Он один на устройство и хранит напоминания ОБЕИХ компаний.
 *  Снося его при переходе, мы получали ровно то, что уже чинили с другой
 *  стороны: сверка реестра с расписанием видела пустой список и гасила
 *  выставленные руками напоминания обеих компаний. То есть `suspend` с пути
 *  перехода я снял, а реестр под ним всё равно продолжал исчезать.
 *
 *  На выходе из АККАУНТА он уходит: там может быть другой человек. */
const PERSON_SCOPED_KEY_PREFIXES: readonly string[] = [
  "babun:notifications:logical",
  // Привычки УСТРОЙСТВА, а не компании: как отсортирован список клиентов,
  // какие поля в карточке, какой день в фильтре, какие подсказки уже закрыты,
  // куда в прошлый раз переводили. Снося их при переходе, чистка возвращала
  // сортировку к «недавним», карточку к умолчаниям и показывала закрытые
  // подсказки заново — при том что ни одна из них не содержит данных
  // компании. Нашёл инвентарь панели 2026-09-13.
  "babun-clients-sort",
  "babun-client-card-fields",
  "babun-clients-filter-day",
  "babun-clients-import-resume",
  "babun:hint-",
  "babun:transfer:last:",
];
...
export function isTenantScopedKey(key: string): boolean {
  return (
    TENANT_SCOPED_KEY_PREFIXES.some((p) => key.startsWith(p)) ||
    PERSON_SCOPED_KEY_PREFIXES.some((p) => key.startsWith(p))
  );
}
