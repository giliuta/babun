...
// СКРУГЛЕНИЯ. Прямоугольные поверхности — 10 (LOCKED 2026-08-22), и КНОПКА
// тоже (уточнено 2026-09-10).
//
// Здесь двадцать строк держали исключение «главная кнопка — 16» с расчётом
// суперэллипса и живым сравнением шести радиусов на симуляторе. Исключение
// умерло молча: токен `cta` не читал НИКТО, `GradientButton` всё это время
// брал `card`. А комментарий остался — и был опаснее мёртвого кода, потому
// что читался как канон: «третья цифра, которую агент завтра возьмёт».
//
// Расхождение владелец увидел с другой стороны — на листе свободного слота,
// где кнопка второго вида стояла пилюлей (999) над прямоугольной главной:
// «квадратная кнопка „Клиент“, круглая „Событие“ — зачем, надо сводить всё
// к одному». Сведено: у любой кнопки `card`.
//
