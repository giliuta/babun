...
// ОДНО СКРУГЛЕНИЕ НА ВСЁ (LOCKED 2026-08-22; исключение для CTA заведено и
// снято 2026-08-27 в один день).
//
// `cta: 16` прожил час и удалён расчётом. `borderCurve: "continuous"` — это
// суперэллипс: кривая уходит от вершины угла вдоль каждого ребра примерно на
// 1.53·R. На кнопке высотой 52 прямой участок короткой стороны = 52 − 3.06·R:
//   R=16 → 3.1pt (6% ребра)  — глаз читает КАПСУЛУ
//   R=10 → 21.4pt (41% ребра) — прямоугольник с загнутыми углами
// Порог перехода — R ≤ 11.3. То есть 16 просьбу владельца «менее круглой,
// немного более квадратные» НЕ выполнял, хотя выглядел компромиссом.
//
// Второй довод сильнее первого: нижний лист скругляется тем же `card` = 10.
// Кнопка 16 внутри листа 10 — ребёнок круглее родителя, ровно та ошибка
// вложенных радиусов, против которой iOS 26 ввела концентричные углы.
//
