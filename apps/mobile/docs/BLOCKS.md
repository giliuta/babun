...
| **Настройки списка** — в шапке шторки | `Settings` (шестерёнка) |
...
Две шестерёнки — не небрежность, а разные роли: `Settings2` правит НАСТРОЙКИ
ЭТОГО БЛОКА (типы объектов, типы событий), `Settings` ведёт на страницу
СПИСКА, из которого выбирают. Владелец 2026-09-08 присылал картинкой именно
ползунки для блока — не путать.
