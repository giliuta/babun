...
| **Настройки блока** — в шапке `RowGroup` / `ChoiceRow` | `Settings2` (два ползунка) |
...
`src/features/clients/ObjectFields.tsx` — три карточки: «Тип объекта»
(`ReferenceBlock` + `PickerSheet`, дверь в справочник — ползунками в шапке
ШТОРКИ, как у типа события), «Адрес» (главная строка + карта `MapPicker` +
...
«Добавить объект» под выбранным; ЛЕНТЫ ЧИПОВ для типа объекта (была до
2026-09-10 — третий диалект выбора рядом с категорией и типом события).
...
Пустой каталог не тупик: пустое состояние — только слова, а кнопка футера
становится **«Добавить услугу»**. Она паркует шторку (`useSheetDoorway`) и
открывает `/services` с командой записи; по «назад» шторка встаёт на место уже
с услугами и «Применить».

**Шторка «Итого»** — `src/features/appointments/TotalSheet.tsx`, ДВА БЛОКА
(вид владельца от 20.09; попытка свести всё в одну таблицу 22.09 отвергнута:
«я уже разработал это правильно — откати»):

```
┌ УСЛУГА        КОЛ-ВО   ЦЕНА   СУММА ┐   первый блок: работы
│ Refilling     − 3 +     60     180  │
│ [Скидка]  10 %     −€18       €162  │   ← ServicesRow — последняя строка перечня
└─────────────────────────────────────┘
┌ ИТОГО   СТАВКА   НАЛОГ    К ОПЛАТЕ  ┐   второй блок: деньги
│ [VAT]   +19 %   €30,78    €192,78   │   ← PayRow
└─────────────────────────────────────┘
```

Строки — `ServicesRow` и `PayRow` в `src/features/appointments/VatLooks.tsx`.

ОДНА ШТОРКА НА ЗАПИСЬ, ЧЕК И ИНВОЙС — с 22.09 у всех трёх скидка и VAT:

| Где | Скидка | VAT | Куда налог уходит |
|---|---|---|---|
| Запись (`app/book/index.tsx`) | да | да | `appointments.vat_mode` + `vat_rate`; оплата ложится на счёт с налогом по ставке записи (`reconcile_appointment_finance`, миграция 20260922040000) |
| Чек (`src/features/documents/ReceiptComposer.tsx`) | да | да | снимок `vat_rate` + `vat_amount` на проводке |
| Инвойс (`src/features/invoices/InvoiceBlocks.tsx`) | да — строкой счёта с флагом `discount` | да | `vat_percent` документа; инвойс из записи берёт её налог |

Порядок денег один: услуги → скидка → VAT → К оплате. VAT: клавиша по кругу
(сверху цены → внутри цены → без налога), ставку ПИШУТ ЦИФРАМИ, как скидку,
пересчёт на каждом символе. Написанная ставка запоминается на устройстве по
компании (`src/features/finances/remembered-vat-rate.ts`) и подставляется в
следующий документ; стартовая — 0.

У инвойса вторая строка услуги — её описание, у записи — длительность: у
счёта нет минут.
...
**Своя услуга — в записи, чеке и инвойсе** — в шторке «Итого» (`TotalSheet`, пропы `onAddLine`
и `onNameChange`): значок «добавить строку» справа в шапке заводит строку,
имя, количество, цена за штуку и сумма правятся прямо в таблице, при закрытии
она уже стоит в блоке «Услуги» с названием; пустая уходит. Тап по своей строке
в блоке снова открывает «Итого» (владелец 22.09). Отвергнуты: форма прайса,
«＋» внизу шторки выбора, строка-анкета прямо в блоке.
У записи и чека строка хранится в снимке услуг с id `custom:<uuid>`
(`isCustomServiceId`, `packages/shared/src/local/appointments.ts`): имя, цена и
время 0 — снимком, прайса у неё нет, смена команды её не уносит, в инвойс из
записи она приходит своей строкой.

**Не делать:** «Добавить услугу» строкой под выбранными; дублировать
длительность в «Итого»; галку слева в строке шторки; ряд готовых ставок VAT
вместо поля (отвергнут 22.09: «не выбор ставки — чтоб я сам написал»);
свою шторку позиций у документа (у инвойса была — `InvoiceLines`, снесена
22.09).
...
**Канон записи:** `app/book/index.tsx` — `SectionCard title="Заметка"` +
`InlineNoteField`. **Канон события:** там же ниже — ТОТ ЖЕ блок и то же поле,
отличаются только подсказка и то, во что оно пишет.

```tsx
<SectionCard title="Заметка">
  <InlineNoteField note={note} placeholder="Заметка записи" accessibilityLabel="Заметка записи" />
</SectionCard>
```

Подсказка НАЗЫВАЕТ поле именем сущности — «Заметка записи», «Заметка события»,
«Заметка клиента», «Заметка объекта», — а не объясняет примером (владелец
2026-09-10). Высота у всех одна: строка в покое, до четырёх при наборе.
Отдельного «высокого» поля у события больше нет — проп `tall` снят.
...
**Порядок — ручкой В САМОМ СПИСКЕ** (владелец 2026-09-12: «шесть точек справа
для передвижения… везде одно и то же»). Шесть точек справа, `ReorderList` с
`handleInside` (строка ещё и смахивается, и «Удалить» обязано упереться в
ручку, а не уехать под неё). Отдельной страницы «порядок» не заводим ни одному
справочнику: последняя, `/accounts/order`, снесена 2026-09-12 — экран ради
одного жеста был лишней дверью. И порядок задаёт ТОЛЬКО `position`:
сортировать сперва по «виду» (как делали счета), а рукой лишь внутри вида —
значит отобрать жест у того, у кого в каждом виде по одной строке.
...
## 9.0 Блок «Вид» — цвет и значок сущности

Один вопрос на весь продукт: **как эта штука узнаётся в списке**. Два поля со
своими решётками (`ColorField`, `IconField`) отменены 2026-09-10 — теперь это
образец в строке и одна шторка с переключателем.

| Что | Файл | Роль |
|---|---|---|
| `AppearanceTile` | `src/components/ui/AppearanceSheet.tsx` | Образец: квадрат цвета, внутри значок. Пусто — тихий ярлычок, а не серая дыра |
| `AppearanceSheet` | `src/components/ui/AppearanceSheet.tsx` | Шторка «Вид»: `SegmentedControl` «Значок · Цвет» → решётка → «Применить» в футере |
| `AppearanceField` | `src/components/ui/picker-fields.tsx` | Строка формы: ярлык, образец, шеврон. Для сущностей БЕЗ имени в этой же строке (счёт) |
| `NameColorField` | `src/components/ui/picker-fields.tsx` | Имя + вид ОДНОЙ строкой: образец слева, поле имени справа. Так у услуги, метки, тега, категории, типа события и типа объекта |
| `ColorPicker` · `IconPicker` | `src/components/ui/ColorPicker.tsx`, `src/components/ui/IconPicker.tsx` | Сами решётки. Вне шторки «Вид» их не зовут |
| `PRESET_COLORS` · `ICON_PRESETS` | `packages/shared/src/common/utils/colors.ts`, `src/components/ui/icon-set.ts` | Сорок цветов и сорок значков. Своих наборов у экранов нет |

Геометрия — `src/components/ui/picker-grid.ts`: 8 столбцов, плитка во всю
клетку (радиус 10), зазор 6. Сорок помещаются в пять рядов, то есть в шторку
на полэкрана без прокрутки.

**СТРОКА СПРАВОЧНИКА ЗАЛИВАЕТСЯ ЦВЕТОМ СУЩНОСТИ** (владелец 2026-09-10:
«сделай подсветку блоков — мне кажется, это более интересное»). Числа —
`appearanceRowFill` из того же файла: 8 % в покое, 14 % под пальцем, без цвета
строка остаётся на своей подложке. Это те же проценты, что у строки шторки
выбора (`select-rows`), поэтому один и тот же тег выглядит одинаково в кабинете
и в выборе. Заливку держит ВСЯ строка: вложенная вторая складывается с первой,
и колонка ручки выходит светлее остального.

Как звать (сущность с именем):
...
<NameColorField
  ...
  icon={icon}                 // без этих двух пропов переключателя не будет
  onIconChange={setIcon}
  icons={EVENT_TYPE_ICON_PRESETS} // только там, где слаги свои
/>
```

**ЗНАЧОК СНИМАЕТСЯ ПОВТОРНЫМ ТАПОМ** (владелец 2026-09-10: «я могу ещё раз
нажать на значок и отменить его — значок это не обязательная штука»). Снятие
живёт В РЕШЁТКЕ (`IconPicker` отдаёт `null`), а не у каждого вызывающего
по-своему: до этого счёт снимал сам, а тег и тип объекта не умели вовсе.
Цвет так НЕ снимается: он держит заливку строки, и строка без цвета выпала бы
из общего ряда.

Чего не делать:

- не ставить распахнутую решётку прямо в форму — сорок плиток забивают лист;
- не заводить своей палитры и своего набора глифов ни одному экрану;
- не мигрировать старые цвета тенанта: пикер покажет их плиткой «свой»;
- не рисовать выбранное кольцом или галочкой снаружи плитки — галка стоит
  ВНУТРИ, тоном, который читается на этой заливке.

Носители вида: счёт, категория финансов, тип события, услуга, тег, метка, тип
объекта. Новый справочник берёт этот же блок. У КОМАНДЫ значка пока нет —
колонка `teams.icon` заведена, клиентская половина не сделана (2026-09-10).
...
| `app/(dashboard)/cabinet/`: `app/(dashboard)/cabinet/inventory.tsx`, `app/(dashboard)/cabinet/templates.tsx`, `app/(dashboard)/cabinet/recurring.tsx`, `app/(dashboard)/(home)/calendar/masters/index.tsx`, `app/(dashboard)/cabinet/account.tsx` | сырой `Modal animationType="slide"` вместо `BottomSheet` | рецепт «Метки» |
