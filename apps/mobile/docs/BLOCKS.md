...
Услуга/Кол-во/Цена/Сумма, под ней строка «Скидка» (евро или процент + число →
«−€5» → сумма после скидки), ниже строка «Итого · Ставка · Налог · К оплате»
(`PayRow` в `src/features/appointments/VatLooks.tsx`).

ОДНА ШТОРКА НА ЗАПИСЬ, ЧЕК И ИНВОЙС (владелец 2026-09-22: «сравни и сделай —
оно должно быть как единое всё»). Разница только в данных:

| Где | Скидка | VAT |
|---|---|---|
| Запись (`app/book/index.tsx`) | да | пока нет (следующий шаг) |
| Чек (`src/features/documents/ReceiptComposer.tsx`) | да | да |
| Инвойс (`src/features/invoices/InvoiceBlocks.tsx`) | да — строкой счёта с флагом `discount` | да |

VAT: клавиша «VAT» по кругу (сверху цены → внутри цены → без налога), ставку
ПИШУТ ЦИФРАМИ в колонке «Ставка», как скидку, — пересчёт на каждом символе.
Написанная ставка запоминается на устройстве по компании
(`src/features/finances/remembered-vat-rate.ts`) и подставляется в следующий
документ; стартовая — 0. Налог всегда считается ПОСЛЕ скидки.
