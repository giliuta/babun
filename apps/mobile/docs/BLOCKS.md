...
**Своя услуга инвойса** — строкой прямо в блоке «Услуги»: «＋ Добавить» в шапке
блока (`ServicesBlock`, пропы `action` и `extra`) добавляет строку
`src/features/invoices/CustomServiceRow.tsx` — название, количество, цена за
штуку и сумма, цена и сумма пересчитываются друг из друга
(`src/features/invoices/custom-line.ts`). Ни шторки, ни прайса (владелец 22.09:
«ничего не создавать — сразу внизу пишу услугу, количество, цену и сумму»).
Форма прайса и «＋» внизу шторки выбора для этого отвергнуты.
