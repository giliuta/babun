...
**Форма услуги** — `src/features/services/ServiceSheet.tsx`, одна на прайс и на
«Свою услугу» документа (владелец 22.09: «возьми чётко тот блок, который мы
используем в создании нормальной услуги»). В инвойсе её открывает «＋» справа
от «Применить» в шторке выбора услуг (`ServicePicker`, проп `onAddCustom`):
режим `oneOff` отдаёт строку в документ, а не в прайс, и прячет параметры
календаря (график, перерыв, онлайн). Тап по своей строке в блоке — та же форма.
