...
import { isoWeekdayOf } from "./date-utils";

describe("ISO-номер дня недели", () => {
  test("понедельник — 1, воскресенье — 7", () => {
    // 2026-08-31 — понедельник, 2026-09-06 — воскресенье.
    assert.equal(isoWeekdayOf("2026-08-31"), 1);
    assert.equal(isoWeekdayOf("2026-09-01"), 2);
    assert.equal(isoWeekdayOf("2026-09-05"), 6);
  ...
  test("ВОСКРЕСЕНЬЕ НЕ ПРЕВРАЩАЕТСЯ В НОЛЬ", () => {
    ...
    assert.notEqual(isoWeekdayOf("2026-09-06"), 0);
  });
  ...
  test("дата разбирается как календарная, без сдвига зоной", () => {
    ...
    assert.equal(isoWeekdayOf("2026-09-06"), 7);
    assert.equal(isoWeekdayOf("2026-01-01"), 4);
  });

  test("мусор не роняет календарь", () => {
    assert.equal(isoWeekdayOf(""), 1);
    assert.equal(isoWeekdayOf("не дата"), 1);
  });
});
