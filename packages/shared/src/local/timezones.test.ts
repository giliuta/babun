...
describe("группы часовых поясов", () => {
  ...
  // СВЕРКИ ПОВЕДЕНИЯ — ТОЛЬКО НА МАШИНЕ РАЗРАБОТЧИКА. Словарь сгенерирован из
  // ICU той машины, и сверка спорит с ДАННЫМИ, а не с кодом: на раннере CI
  // (Linux, системный tzdata посвежее) Ванкувер и Эдмонтон ведут себя как
  // Лос-Анджелес и Денвер, а одна из зон UTC+3 переводит часы — и словарь
  // «разъезжается» с любым tzdata, кроме своего. Обновили bun (а с ним ICU) —
  // перегенерируйте словарь, эти проверки покажут, что именно поплыло.
  // 63 группы × города × 365 проб Intl — 4,5 с; штатных 5 с впритык.
  test.skipIf(!!process.env.CI)("каждый город группы валиден и ведёт себя как её представитель", () => {
  ...
  test.skipIf(!!process.env.CI)("ГЛАВНОЕ: две группы никогда не ведут себя одинаково круглый год", () => {
